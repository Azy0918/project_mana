...
TITLE_BY_CUT = {
    ...
    9: "ナビとの接続", 10: "在庫予測モデル", 11: "欠けた履歴メモ", 12: "足りない条件",
...
# Final cut line -> image vc. vc20 is intentionally unused for now; the last
# dialogue beat returns to the cleaning image instead of the night-shift wide.
PER_LINE_IMAGE_VC = {
    "ep02_v059": 16,  # 冷凍庫の青いラベルが光る
    "ep02_v060": 17,  # もう次回予告が冷凍庫から来てる
    "ep02_v061": 17,  # 先に掃除
    "ep02_v062": 17,  # 未来より掃除
...
EP02_IMAGE_PROMPTS = {
    ...
    4: ("深夜コンビニ店内。タクミが前話の時空返品レシートを透明袋に保管している。エリは淡々と見守る。制服、髪型、名札位置を固定。", ["takumi", "mina"]),
    5: ("自動ドアが開き、汗田が紫ヘルメットを持ってコンビニに入る。タクミとエリがカウンター付近で対応する。汗田、タクミ、エリの顔と制服を固定。", ["aseda_ryuji", "takumi", "mina"]),
    6: ("午前二時十七分、第十三レジが現れる。黒いセルフレジ端末の眠そうなシアン顔。汗田は状況を観察し、タクミは困惑、エリは平常運転。", ["thirteenth_register", "aseda_ryuji", "takumi", "mina"]),
    ...
    11: ("タクミとエリが汗田の判断を見守る。第十三レジは無表情に処理を待つ。制服と髪型を固定し、タクミとエリを別人化しない。", ["takumi", "mina", "aseda_ryuji", "thirteenth_register"]),
    ...
    14: ("エリがブラックコーヒーを一杯だけ汗田へ渡す。汗田はまだコーヒーを持っておらず、受け取ろうとしている。二重のコーヒーを描かない。", ["mina", "aseda_ryuji", "takumi"]),
    ...
    17: ("閉店前ではなく深夜営業中の通常業務へ戻る。タクミとエリが清掃用具の前で次の作業を確認する。顔、髪型、制服を固定。", ["takumi", "mina"]),
    ...
    20: ("深夜コンビニの通常業務に戻る。タクミとエリ、床清掃、棚、レジ周り。大事件の後でも普通の夜勤が続く空気。", ["takumi", "mina", "thirteenth_register"]),
}
