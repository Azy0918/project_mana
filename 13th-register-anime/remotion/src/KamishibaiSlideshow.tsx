...
const getCutStartFrames = (cuts: KamishibaiCut[]) =>
  cuts.reduce<number[]>((starts, cut, index) => {
    if (index === 0) {
      return [0];
    }
    const previous = starts[index - 1] + Math.round(cuts[index - 1].duration * fps);
    return [...starts, previous];
  }, []);

const getCurrentCut = (cuts: KamishibaiCut[], cutStartFrames: number[], frame: number) => {
  let index = 0;
...
const FrameImage: React.FC<{
  ...
  location: string;
  characters: string;
}> = ({ imageBase, image, index, progress, caption, location, characters }) => {
...
export const KamishibaiSlideshow: React.FC<{ cuts: KamishibaiCut[]; imageBase: string }> = ({
  cuts,
  imageBase,
}) => {
  const frame = useCurrentFrame();
  const cutStartFrames = getCutStartFrames(cuts);
  const { cut, index, local, duration } = getCurrentCut(cuts, cutStartFrames, frame);
