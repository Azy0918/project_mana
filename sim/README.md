...
### ISMCTSをGA本走に載せる(二段GA)

`evolve_meta.evolve_two_tier`: Tier1=Heuristicで全個体を高速ランク → Tier2=ISMCTSで
上位K体だけを忠実評価しエリート選抜に使う(評価はキャッシュ)。ISMCTSは高コストなので
有望上位のみに使い、現実的時間(6世代3〜6分)で回す。ループ内ISMCTSは `fast_ismcts_pilot`
(40反復, フルの約2倍速)。

**知見**: ループ内ISMCTSが弱すぎ(24反復)るとメタの操縦が甘くアグロを過大評価し、GA最良が
フル評価で崩壊(適応度0.54→フル0.37)。予測力のある強さ(40反復)にすると適応度≒フル評価
(0.50→0.44, ギャップ0.06)で**GAが正しいものを最適化**する。いずれにせよ ISMCTS をループに
入れると、発見デッキは対メタ約0.5止まり=**メタを超えるデッキは存在しない**ことを再確認
(Heuristicの幻0.62〜0.78は正しく割り引かれる)。フック早期脱出等で全対戦も高速化済み。
