...
## 2026-06-30 タイミング修正 v2

- ユーザー指定により、v012 は `vc03 夜の国道へ` を表示する。
- ユーザー指定により、v019-v022 は汗田がコンビニに入ってくる旧入店絵 `ep02_vc05_aseda_enters.png` を表示する。
- 現行の `ep02_vc05_aseda_enters_eri_v1.png` は第十三レジ前の会話絵で、入店カットとしては使わない。
