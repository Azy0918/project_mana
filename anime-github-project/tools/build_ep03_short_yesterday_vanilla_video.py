# -*- coding: utf-8 -*-
"""第3話ショート(縦9:16/1080x1920/30fps/BGMなし)を生成する専用スクリプト。

本編素材は変更せず、scene_manifest_ep03 のタイムコードで既存フル音声を
切り出してショート用に再構成する。
出力: video/ep03_short_yesterday_vanilla.mp4
"""
...
PAUSE = 0.06
END_DUR = 1.6
OUT = REPO / "video" / "ep03_short_yesterday_vanilla.mp4"
WORK = REPO / "outputs" / "shorts"
AUDIO_OUT = WORK / "ep03_short_yesterday_vanilla_audio.wav"
ASS_OUT = REPO / "ep03_short_yesterday_vanilla_subs.ass"
...
ITEMS = [
    ...
    {"line": "ep03_v005"},
    {
        "visual": "ice_label",
        "dur": 2.2,
        "image": "assets/scenes/planned/ep03_vc02_yesterday_vanilla_cup_yvtm_v1.png",
    },
    {"line": "ep03_v017"},
    {"line": "ep03_v019"},
    {"line": "ep03_v020"},
...
def mix_at(base: np.ndarray, add: np.ndarray, at_sec: float, volume: float = 0.22) -> np.ndarray:
...
def main() -> int:
    scenes = {s["id"]: s for s in json.loads((KAMI / "scene_manifest_ep03.json").read_text(encoding="utf-8"))}
    source_audio = read_wav_pcm(KAMI / "assets" / "ep03_full_voice_reading_hiragana.wav")
    ...
    if SFX.exists() and bag_sfx_at is not None:
        full_audio = mix_at(full_audio, read_wav_pcm(SFX), bag_sfx_at, volume=0.18)
...
"""
    ...
    dlg(0.05, min(4.2, total), "Title", f"{{\\pos({W // 2},540)\\fs86\\c{GREEN}\\b1}}{HOOK}")
    dlg(seg("ice_label")["start"], seg("ice_label")["end"] + 0.2, "Pop",
        f"{{\\pos({W // 2},520)\\fs66\\b1}}昨日バニラ\\N{{\\fs48\\c{GREEN}}}注意：溶けると昨日になります")
    dlg(seg("ep03_v042")["start"], seg("ep03_v062")["end"] + 0.2, "Pop",
        f"{{\\pos({W // 2},500)\\fs58\\c{GREEN}\\b1}}掃除記録、昨日状態\\N{{\\fs52}}努力が昨日に食われた")
    dlg(seg("ep03_v050")["start"], seg("ep03_v053")["end"] + 0.2, "Pop",
        f"{{\\pos({W // 2},500)\\fs64\\c{GREEN}\\b1}}時間をレジ袋で分ける？")
    dlg(seg("ep03_v054_g1")["start"], seg("ep03_v057")["end"] + 0.2, "Pop",
        f"{{\\pos({W // 2},500)\\fs62\\c{GREEN}\\b1}}レジ袋が時空より強い！")
    ...
    print(f"build ep03 short: {len(segments)} items, total={total:.2f}s -> {OUT}", flush=True)
