...
VOICE_MANIFEST = REPO_ROOT / "13th-register-kamishibai" / "assets" / "manifest_reading_hiragana_ep08.json"
TIMELINE = REPO_ROOT / "outputs" / "ep08_voice_reading_hiragana" / "ep08_full_voice_reading_hiragana_timeline.json"
DEFAULT_ASSIGNMENT = REPO_ROOT / "ep08_image_assignment_proposal.json"
OUT_PATHS = [
    REPO_ROOT / "13th-register-kamishibai" / "scene_manifest_ep08.json",
    REPO_ROOT / "site" / "scene_manifest_ep08.json",
]

# Claude-owned (台本): visual-cut titles keyed by the IMAGE vc number (Codex's 20 EP08 beats).
TITLE_BY_VC = {
    1: "発注端末が震える", 2: "酸素グミ一万個", 3: "月面店の発注データ", 4: "補給線の計算", 5: "危険な物流設計",
    6: "三時間後に納品", 7: "三列まで", 8: "棚は安全装置", 9: "唐沢、発注確認", 10: "現実の数字で宇宙を殴る",
    11: "二十四個へ補正", 12: "月面店から通信", 13: "一万個ないと不安", 14: "不安を在庫で埋めない", 15: "仮発注を確定",
    16: "レシート", 17: "特殊レジ施策？", 18: "棚割りのマニュアル", 19: "酸素グミは空気味", 20: "三列の通常営業",
}
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build episode 8 scene manifest from the voice manifest, audio timeline and image assignment."
    )
