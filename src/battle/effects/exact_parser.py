...
_SAFE_BODY_PATTERNS = [
    ...
    r"^自分の(?:墓地|マナゾーン|山札)から.{0,40}(?:超)?探索し.{0,40}(?:手札に(?:戻す|加える|戻し|加え)|バトルゾーンに出(?:す|してもよい)|唱える|唱えてもよい|公開(?:する|してもよい))(?:てもよい)?$",
    ...
    # 相手の墓地リセット(山札に戻してシャッフル): 墓地利用妨害=有益=under-model=安全
...
def _detect_trigger(clause: str) -> tuple[str | None, str]:
    ...
    # under-model(安全)。
    m = re.match(r"^このクリーチャー、または自分の他の.{1,40}がバトルゾーンに出た時[、,]?(.+)$", cl)
    if m:
        return ("on_play", m.group(1))
